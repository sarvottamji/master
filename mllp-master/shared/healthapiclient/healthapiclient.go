...
func (c *HL7V2Client) Send(data []byte) ([]byte, error) {
	...
	log.Infof("Received message of %s . Sending this message to the Cloud Healthcare API HL7V2 Store.", (data))
	...
	if err != nil {
		...
		if e, ok := err.(*googleapi.Error); ok {
			...
			if nack != nil {
				log.Errorf("Message was sent to the Cloud Healthcare API HL7V2 Store, received a NACK response.The original message was %s",(data))
				if c.logNACKedMsg {
